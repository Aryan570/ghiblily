...
const Blogs = async () => {
    ...
    const truncate_text = (text : string, len : number) => {
    ...
    return (
        <div className='min-h-screen hero w-full flex flex-col justify-center items-center quicksand'>
            <div className='font-bold w-1/2 text-2xl mb-2 left-0 font-right'>Welcome to the rock</div>
            {blogs.map((blog) => (
                <Link className='md:w-1/2 w-4/5 hover:scale-[1.03] transition-transform duration-300 ease-out mb-2.5 backdrop-blur-xs' key={blog._id} href={blog.link}>
                    <div className='flex border-1 border-slate-400 max-h-1/12 h-[20vh] overflow-hidden'>
                        <div className='w-3/5 justify-start flex flex-col p-4'>
                            <div className='md:text-2xl font-bold overflow-clip underline-offset-2 font-right'>{blog.title}</div>
                            <p className='overflow-clip text-ellipsis'>{truncate_text(blog.description, 50)}</p>
